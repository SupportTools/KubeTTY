# Execute Sprint Tasks - KubeTTY

Execute all tasks in Sprint #$ARGUMENTS
...
Use dynamic lookup to get current sprints:
```
...
1. Query TaskForge for Sprint #$ARGUMENTS to get sprint metadata
2. Get all pending tasks (`status: "todo"`) in the sprint
...
Default: Commit at end of sprint with comprehensive message listing all tasks.
Alternative: Per-task commits if specified by user.

---

Now execute Sprint #$ARGUMENTS following this workflow exactly.
